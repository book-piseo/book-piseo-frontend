...
const RoundButton = ({ type = 'primary', label, disabled = false, onClick }: RoundButtonProps) => {
  ...
  return (
    <div
      className={`flex items-center justify-center w-fit h-[44px] px-[40px] rounded-[30px] ${buttonConfig}`}
      onClick={() => !disabled && onClick && onClick()}
    >
      <span className={`text-[16px] font-semibold ${RoundButtonConfig[type].label}`}>{label}</span>
    </div>
  )
}
